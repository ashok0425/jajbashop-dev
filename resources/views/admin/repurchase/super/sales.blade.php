...
<td>
    <a href="{{route('admin.super.order.show',['id'=>$item->id,'orderId'=>$item->order_id])}}" class="btn btn-info"><i class="fas fa-eye"></i></a>
    <button class="btn btn-danger mr-2 d_in_win openWin" data-id="{{ $item->id }}"><i class="fas fa-print"></i> Print</button>
    <a href="{{route('admin.super.order.print',['id'=>$item->id,'orderId'=>$item->order_id])}}" class="btn btn-danger mr-2 "><i class="fas fa-download"></i> Download</a>
