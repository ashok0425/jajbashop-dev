...
<div class="row">
    <div class="col-md-6">
        <div class="card">
            ...
            <div class="card-body">
                <form id="btnSave" action="{{route('distributor.purchase.store')}}" method="GET">
                    {{ csrf_field() }}
  ...
  <div class="modal fade" id="checkout-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <form action="{{route('distributor.purchase.checkout')}}" method='POST'>
            @csrf
            <select name="super" id="" class="form-control" required>
              <option value="">--select super distributor</option>
              @foreach ($sd as $item)
              <option value="{{ $item->id }}">Name:{{ $item->name}} ,Address: {{ $item->address }} ,{{ $item->city }} , {{ $item->district }}  , Phone: {{ $item->phone }}</option>
              @endforeach
            </select>
...
// fastdropdown
        ...
        $(document).ready(function () {
            $('.product_id').on('change', function (e) {
                e.preventDefault()
                var pid = $(this).val();
                var path = "{{url('distributor/purchase/getproductdata')}}/"+pid;
                $.ajax({
                    url: path,
                    method: 'GET',
                    dataType: 'json',
    ...
    <script>
        ...
        function readsales() {
            $.ajax({
                type: 'get',
                url: "{{url('distributor/purchase/list')}}",
                dataType: 'html',
                success: function (data) {
        ...
        $(document).on('click','.delete-sales',function() {
            ...
            $.ajax({
                type: 'get',
                url: "{{url('distributor/purchase/delete')}}/"+id,
                dataType: 'json',
                success: function (data) {
                    readsales();
