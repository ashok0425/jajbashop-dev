...
<div class="row">
  ...
  <form method="GET" class="reply-forms submit" action="{{ route('product.rating.store') }}">
@csrf
<input type="hidden" name="product_id" value="{{ $id }}">
  ...
  <div class="comment-thread">
    ...
    <div class="comment-body my-3">
        ...
        <!-- Reply form start -->
        <form method="GET" class="reply-form d-none reply" id="comment-{{ $item->id }}-reply-form" action="{{ route('product.rating.reply.store') }}">
          @csrf
        ...
        {{-- fetching reply  --}}
    @php
        $reply=DB::connection('mysql2')->table('replies')->join('users','users.id','replies.user_id')->select('users.name','users.id as uid','replies.*')->where('comment_id',$item->id)->get();
    @endphp
